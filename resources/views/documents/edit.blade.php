                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form action="{{ route('documents.update', $document) }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 gap-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <select id="related_type" name="related_type"
                                        ...
                                        <option value="global" {{ old('related_type', $document->related_type) == 'global' ? 'selected' : '' }}>Global (General)</option>
                                        <option value="task" {{ old('related_type', $document->related_type) == 'task' ? 'selected' : '' }}>Task Specific</option>
                                    </select>
                                    <x-input-error class="mt-2" :messages="$errors->get('related_type')" />
                                </div>

                                <!-- Related Task -->
                                <div x-data="{ show: {{ old('related_type', $document->related_type) == 'task' ? 'true' : 'false' }} }"
                                    ...
                                    <div x-show="show">
                                        ...
                                        <select id="related_id" name="related_id"
                                            ...
                                            @foreach($tasks as $task)
                                                <option value="{{ $task->id }}" {{ old('related_id', $document->related_id) == $task->id ? 'selected' : '' }}>
                                                    {{ $task->title }}
                                                </option>
                                            @endforeach
                                        </select>
                            ...
                            <div>
                                <x-markdown-editor name="content_md" :value="old('content_md', $document->content_md)"
